...
def find():
    ...
    # создание таблицы
            ...
            if combo.get() == 'IP':
                ...
                select_table_query = f"""select * from vsftpd where ip like '%{what_find.get()}';"""
                ...
                for colm in table.get_children():
                    table.delete(colm)
                cursor = conn.cursor(buffered=True)
                select_table_query = f"""select * from vsftpd where data like '%{what_find.get()}';"""
                cursor.execute(select_table_query)
                for colm in cursor.fetchall():
                    table.insert("", 'end', iid=colm[0], values=(colm[0],colm[1],colm[2],colm[3],colm[4]))
                conn.commit()
                ...
                for colm in table.get_children():
                    table.delete(colm)
                cursor = conn.cursor(buffered=True)
                select_table_query = f"""select * from vsftpd where time like '%{what_find.get()}';"""
                cursor.execute(select_table_query)
                for colm in cursor.fetchall():
                    table.insert("", 'end', iid=colm[0], values=(colm[0],colm[1],colm[2],colm[3],colm[4]))
                conn.commit()
            elif combo.get() == 'Статус':
                for colm in table.get_children():
                    table.delete(colm)
                cursor = conn.cursor(buffered=True)
                select_table_query = f"""select * from vsftpd where status like '%{what_find.get()}';"""
                cursor.execute(select_table_query)
                for colm in cursor.fetchall():
                    table.insert("", 'end', iid=colm[0], values=(colm[0],colm[1],colm[2],colm[3],colm[4]))
                conn.commit()
...
def all_view():
    ...
    # создание таблицы
        ...
        for colm in table.get_children():
                table.delete(colm)
        cursor = conn.cursor(buffered=True)
        select_table_query = """select * from vsftpd;"""
        cursor.execute(select_table_query)
        for colm in cursor.fetchall():
            table.insert("", 'end', iid=colm[0], values=(colm[0],colm[1],colm[2],colm[3],colm[4]))
        conn.commit()
...
label_title = Label(window, text="FTP-сервер", font=("Arial Bold", 30), bg="deep sky blue")
...
combo['values'] = ('IP', 'Дата', 'Время', 'Статус')
