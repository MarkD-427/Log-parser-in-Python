...
def find():
    ...
    # создание таблицы
        ...
        if combo.get() == 'Пользователь':
            ...
            select_table_query = f"""select * from auth_openfire where user like '%{what_find.get()}';"""
            ...
            for colm in table.get_children():
                table.delete(colm)
            cursor = conn.cursor(buffered=True)
            select_table_query = f"""select * from auth_openfire where data like '%{what_find.get()}';"""
            cursor.execute(select_table_query)
            # table.delete(all)
            for colm in cursor.fetchall():
                table.insert("", 'end', iid=colm[0], values=(colm[0],colm[1],colm[2],colm[3]))
            ...
            for colm in table.get_children():
                table.delete(colm)
            cursor = conn.cursor(buffered=True)
            select_table_query = f"""select * from auth_openfire where time like '%{what_find.get()}';"""
            cursor.execute(select_table_query)
            # table.delete(all)
            for colm in cursor.fetchall():
                table.insert("", 'end', iid=colm[0], values=(colm[0],colm[1],colm[2],colm[3]))
...
label_title = Label(window, text="Авторизация Openfire", font=("Arial Bold", 30), bg="deep sky blue")
