...
def find():
    ...
    # создание таблицы
        ...
        if combo.get() == 'Пользователь':
            ...
            select_table_query = f"""select * from auth_ftp where user like '%{what_find.get()}';"""
            ...
            for colm in table.get_children():
                table.delete(colm)
            cursor = conn.cursor(buffered=True)
            select_table_query = f"""select * from auth_ftp where data like '%{what_find.get()}';"""
            cursor.execute(select_table_query)
            # table.delete(all)
            for colm in cursor.fetchall():
                table.insert("", 'end', iid=colm[0], values=(colm[0],colm[1],colm[2],colm[3]))
            ...
            for colm in table.get_children():
                table.delete(colm)
            cursor = conn.cursor(buffered=True)
            select_table_query = f"""select * from auth_ftp where time like '%{what_find.get()}';"""
            cursor.execute(select_table_query)
            # table.delete(all)
            for colm in cursor.fetchall():
                table.insert("", 'end', iid=colm[0], values=(colm[0],colm[1],colm[2],colm[3]))
...
def all_view():
    ...
    # создание таблицы
        ...
        for colm in table.get_children():
                table.delete(colm)
        cursor = conn.cursor(buffered=True)
        select_table_query = """select * from auth_openfire;"""
        cursor.execute(select_table_query)
        for colm in cursor.fetchall():
            table.insert("", 'end', iid=colm[0], values=(colm[0],colm[1],colm[2],colm[3]))
        conn.commit()
...
label_title = Label(window, text="Авторизация FTP-сервера", font=("Arial Bold", 30), bg="deep sky blue")
...
try:
    # создание таблицы
    cursor = conn.cursor(buffered=True)
    select_table_query = """select * from auth_ftp;"""
    cursor.execute(select_table_query)
    for colm in cursor.fetchall():
        table.insert("", 'end', iid=colm[0], values=(colm[0],colm[1],colm[2],colm[3]))
    conn.commit()
