...
def find():
    ...
    # создание таблицы
        conn = create_connection_mysql_db(db_config["mysql"]["host"],
                                    ...
                                    "parser_db")
        if combo.get() == 'Пользователь':
            for colm in table.get_children():
                table.delete(colm)
            cursor = conn.cursor(buffered=True)
            select_table_query = f"""select * from auth_pochta where user like '%{what_find.get()}';"""
            cursor.execute(select_table_query)
            # table.delete(all)
            for colm in cursor.fetchall():
                table.insert("", 'end', iid=colm[0], values=(colm[0],colm[1],colm[2],colm[3]))
            conn.commit()
        elif combo.get() == 'Дата':
            for colm in table.get_children():
                table.delete(colm)
            cursor = conn.cursor(buffered=True)
            select_table_query = f"""select * from auth_pochta where data like '%{what_find.get()}';"""
            cursor.execute(select_table_query)
            # table.delete(all)
            for colm in cursor.fetchall():
                table.insert("", 'end', iid=colm[0], values=(colm[0],colm[1],colm[2],colm[3]))
            ...
            for colm in table.get_children():
                table.delete(colm)
            cursor = conn.cursor(buffered=True)
            select_table_query = f"""select * from auth_pochta where time like '%{what_find.get()}';"""
            cursor.execute(select_table_query)
            # table.delete(all)
            for colm in cursor.fetchall():
                table.insert("", 'end', iid=colm[0], values=(colm[0],colm[1],colm[2],colm[3]))
...
def all_view():
    ...
    # создание таблицы
        ...
        for colm in table.get_children():
                table.delete(colm)
        cursor = conn.cursor(buffered=True)
        select_table_query = """select * from auth_pochta;"""
        cursor.execute(select_table_query)
        for colm in cursor.fetchall():
            table.insert("", 'end', iid=colm[0], values=(colm[0],colm[1],colm[2],colm[3]))
        conn.commit()
...
frame_founder.place(x=20, y=100, relwidth=0.4)
frame_results.place(relx=0.5, y=100, relwidth=0.4)
# frame_founder.grid(column=0, row=0, padx=50)
# frame_results.grid(column=1, row=0, padx=50)

label_title = Label(window, text="Авторизация на почте", font=("Arial Bold", 30), bg="deep sky blue")
label_title.grid(column=0, row=0)

scrollbar = Scrollbar(frame_results)
scrollbar.pack( side = RIGHT, fill = Y )
table = ttk.Treeview(frame_results, yscrollcommand = scrollbar.set )
...
table.heading("2", text="data")
table.heading("3", text="time")
...
combo['values'] = ('Пользователь', 'Дата', 'Время')
combo.current(0)
